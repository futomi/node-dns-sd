...
*   - filter | String    | Optional | If specified, this method discovers only devices
*                                     which the string is found in the `fqdn`, `address`,
*                                     `modelName` or `familyName`
* ---------------------------------------------------------------- */
...
DnsSd.prototype._evaluateDeviceFilter = function(device, filter) {
	if(device['fqdn'].indexOf(filter) >= 0) {
		return true;
	}
	if(device['address'].indexOf(filter) >= 0) {
		return true;
	}
	if(device['modelName'].indexOf(filter) >= 0) {
		return true;
	}
	if(device['familyName'].indexOf(filter) >= 0) {
		return true;
	}
	return false;
};
